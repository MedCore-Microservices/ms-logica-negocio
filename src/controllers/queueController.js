...
// GET /api/queue/doctor/:doctorId/stats
async function stats(req, res) {
  try {
    const { doctorId } = req.params;
    const data = await service.getQueueStats(doctorId);
    res.json({ success: true, data });
  } catch (error) {
    res.status(400).json({ success: false, message: error.message });
  }
...
module.exports = { join, current, history, callNext, complete, position, waiting, cancelTicket, confirmed, stats };
